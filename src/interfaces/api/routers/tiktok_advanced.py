...
def _get_service(session: AsyncSession = Depends(get_db)) -> TikTokAdvancedService:
    return TikTokAdvancedService(repository=TikTokRepository(session))
...
async def growth(
    ...
    service: TikTokAdvancedService = Depends(_get_service),
):
