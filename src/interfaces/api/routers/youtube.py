...
def _get_service(session: AsyncSession = Depends(get_db)) -> YouTubeAnalyticsService:
    return YouTubeAnalyticsService(repository=YouTubeRepository(session))


@router.get("/{account_id}/overview", response_model=YTOverview)
async def overview(
    account_id: str,
    service: YouTubeAnalyticsService = Depends(_get_service),
):
